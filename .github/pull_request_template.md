...
- 병합은 rebase 병합이다(DEC-20). 스프린트 PR은 CI 필수 체크 초록 + 다른 세션의 적대적 리뷰 승인 시 병합 담당 AI가, PRD·D1~D4 영향·권한 경계 변경(V1-01 권한 경계, V1-00 DEC 행, `.github/CODEOWNERS`, 이 템플릿, `AGENTS.md`, `CLAUDE.md`)·소유자 보류 PR은 소유자가 병합한다(V1-01 '에이전트 권한 경계' 병합 행).
...
- [ ] 적대적 리뷰 승인(구현 에이전트와 다른 세션). PRD·D1~D4 영향, 권한 경계 변경 또는 소유자 보류 PR이면 **소유자 검토·병합**
