...
const schema = z.object({ id: z.string().min(1), status: z.enum(['pending', 'in_progress', 'completed', 'rejected']) });
...
export async function POST(request: NextRequest) {
  try {
    ...
    await requestRef.set(
      { status: input.status, updatedAt: FieldValue.serverTimestamp(), updatedBy: actor.uid },
      { merge: true },
    );
    await writeAudit(actor, 'request.status.update', { requestId: input.id }, { status: input.status });
