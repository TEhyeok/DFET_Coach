...
| TL-02 | `tool/backlog/labels.json` | 라벨 68개(`scope/mvp` 포함) |
...
에이전트가 할 수 없는 운영·콘솔 작업이다(01 에이전트 권한 경계, DEC-15). 위에서부터 순서대로 한다. 값(앱 ID, 키, 버킷 이름, 이메일, uid)은 저장소·PR·이슈에 적지 않는다. MVP 흐름의 서울 확인은 이 목록이 끝나야 할 수 있고, 그 전에는 에뮬레이터로 개발·확인한다(코딩 차단 아님).
...
| 7 | DF-905(MVP 범위) | 서울 DB 재생성 뒤 소유자 계정 하나에 claim `trainer: true`와 `trainers/{uid}`(`memberIds: []`). 운영 가상 회원 시드는 MVP에서 하지 않는다(테스트 회원은 앱에서 만든다) | 소유자 계정으로 Auth 사용자 생성 → 콘솔에서 `users/{uid}.role = "trainer"` → `node functions/scripts/migrate_trainers.js`(dry-run) → `--apply`(서비스 계정 키는 저장소 밖) | S04(DF-012) 전 |
...
| V2(미추정) | 6 | 0 |
| 추가 제안(채택 대기) | 15 | 19 |
| **MVP 범위(`scope/mvp`, DEC-22)** | 65(완료 7, S01 소유자 대기 2, S02~S12 56) | 185(완료 16, 남은 169) |
