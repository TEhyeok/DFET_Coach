...
> **DEC-22(2026-09-25) 뒤 이 달력의 '단계'와 '주요 목표' 열은 DEC-22 이전 전체 계획이다(보존).** 지금은 1인 알파 MVP만 만든다. S02~S13의 약속 항목·목표는 [03 MVP 계획](03_RELEASE_AND_SPRINT_PLAN.md#mvp-계획dec-22)과 `scope/mvp` 라벨이 정본이다. 예: S05 syncRecordAccessKeys·차단 모드, S05 단계 종료 검토(DF-926), S08 운영 배포, S09 MIG-03 적용과 플래그 개방은 MVP 뒤로 연기했다(운영 배포와 테스트 회원용 플래그는 MVP 범위의 소유자 작업 DF-931·DF-925·DF-929로 따로 한다). 기간·작업일·휴일·용량 열은 그대로 쓴다. 연기 항목(`scope/deferred`)은 스프린트에 넣지 않고 작업 지시서도 만들지 않는다.
...
| v1.2 | 2026-09-25 | DEC-22 반영: 스프린트 달력의 단계·주요 목표 열을 DEC-22 이전 계획(보존)으로 표시하고 03 MVP 계획을 정본으로 가리킴. 연기 항목(`scope/deferred`)은 스프린트·작업 지시서 대상이 아님 | claude/docs-dec-2026-09-25 | 없음(개발 운영 규칙) |
