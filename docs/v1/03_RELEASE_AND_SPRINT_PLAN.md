...
> **지금 적용되는 계획이다.** 2026-09-25 [DEC-22](00_README.md#결정-기록)(AI 결정, 소유자 확인 필요: PR #113 소유자 병합)로 범위를 '1인 알파 MVP'로 줄였다. 아래 §4~§9의 P0~P3 계획은 MVP 뒤 계획으로 **연기**하며 내용은 지우지 않는다.
...
- 지금 필요한 것은 소유자(트레이너) 한 명이 **테스트 회원**으로 네 흐름을 끝까지 쓰는 것이다. 그래서 `scope/mvp` 65건·185점만 남긴다: 완료 7건·16점, S01 소유자 대기 2건·2점, S02~S12 56건·167점. S13(12-21~12-24)은 MVP 종료 확인과 버퍼다.
- 연기한 128건은 스프린트 값을 옛 계획 그대로 두고, MVP 종료 검토 뒤 실회원 투입 순서에 맞춰 다시 계획한다.

### 네 흐름과 필요한 항목

| 흐름 | 화면·기능 | 항목(MVP) |
|---|---|---|
| 공통 기반 | 로그인, 셸, 로컬 저장, 동기화, 디자인, 테스트 회원, 동의 ①②③ 기록, 규칙·인덱스·Storage, 시드·통합 CI, 서울 리전 | DF-006, DF-009, DF-012, DF-013, DF-014, DF-015, DF-016, DF-017, DF-018, DF-020, DF-021, DF-022, DF-023, DF-024, DF-027, DF-034, DF-035, DF-037, DF-038, DF-039, DF-042, DF-043, DF-104, DF-107, DF-108, DF-109, DF-110, DF-113 |
...
| ③ 체형 | 정면·측면 촬영(시뮬레이터는 사진 가져오기), EXIF 제거, Vision 랜드마크 + 수동 보정, 산식(CVA·어깨 높이 차 강조), 확정·결과 | DF-200, DF-201, DF-204~DF-210 |
...
- 약속 한도는 §3.2 용량 × 0.9다. S02는 K-19 예외(12/11)를 그대로 둔다. 같은 스프린트 안 의존은 허용한다(02 §4.2).
- 진행 중인 MVP 외 항목(DF-007 회원 앱 코덱, DF-010 금지어 보고 모드, DF-011 static-guards)은 PR이 열려 있어 마친다. DF-120의 금지어 인라인 경고가 DF-010 목록을 쓴다.
...
상태: 완료 = main 병합, 진행 중 = PR 열림(다른 워크플로), 할 일, 소유자 대기. 'MVP 범위'가 '일부'인 카드는 카드의 MVP 절에 미루는 부분이 있다.
...
| DF-037 | Functions shared 구조(region, callable 어댑터 이전, errors, audit)를 만든다 | 진행 중 | S03 | 2 | Functions 공통 구조(region=asia-northeast3, 오류, 인증). recordConsent가 이 위에 선다 | 전체 |
...
| DF-113 | TR-02 회원 목록(대기 배지, 동의 칩, 검색, 대기 회원 추가)을 완성한다 | 할 일 | S07 | 2 | TR-02 회원 목록(대기 배지, 동의 칩, 검색, 대기 회원 추가) | 전체 |
| DF-116 | TR-04 Live(캔버스 60% 이상, 한 줄 입력, '기록 완료' 1탭, 이어쓰기·새 세션 선택)를 구현한다 | 할 일 | S07 | 5 | SOAP 흐름 1: TR-04 Live(Pencil 캔버스, 한 줄 입력, '기록 완료') | 전체 |
...
| DF-209 | 체형 확정, 새 버전(supersedesId), voided, 기준선 규칙을 구현한다 | 할 일 | S12 | 5 | 흐름 3: 체형 확정·새 버전·기준선 | 전체 |
| DF-210 | TR-09 결과 화면과 자세 편위 표를 구현한다('산정 준비 중') | 할 일 | S12 | 3 | 흐름 3: TR-09 결과 화면과 편위 표('산정 준비 중') | 전체 |
...
- 동의 5종 전체, 서명 패드·서명 저장, 철회·오프라인 동의, 권리 행사, 보유기간 파기, 감사 로그, `opsMetrics`. MVP는 대기 회원의 ①②③ 동의 기록만 한다
...
- 분석 이벤트·레지스트리(DF-033·DF-126), 오늘 세션 보드(DF-125), NRS·바디맵(DF-117·DF-219), 이어쓰기(DF-124), O 자동 불러오기·나란히 보기(DF-214·DF-217·DF-218), 재검사·스테이션(DF-203·DF-212), 실기기 프로토콜·접근성 감사(DF-140·DF-141·DF-223·DF-226), TestFlight(DF-139)
...
- [ ] 테스트 회원(대기 회원)을 만들고 ①②③ 동의를 기록한다. 회원 목록에 대기 배지와 동의 칩이 보인다
...
- [ ] ③ 정면·측면 사진을 가져와(시뮬레이터) Vision 랜드마크 제안 → 수동 보정 → CVA·어깨 높이 차 확인(나머지는 '참고') → 확정·기준선. 업로드 사진에 EXIF·GPS가 없다
...
- [ ] 소유자 계정(claim)으로 시뮬레이터 앱에서 A의 네 흐름을 한 번 더 하고, 콘솔에서 문서·파일을 확인한다. 테스트 회원만 쓴다
- [ ] 연결된 iPhone은 회원 앱·BodyPath 확인에만 쓴다. iPad 실기기가 연결되면 체형 촬영(roll·pitch 게이트, 카메라)을 실기기로 다시 확인한다
...
| v1.2 | 2026-09-25 | DEC-22 'MVP 계획' 절 추가(1인 알파 MVP, `scope/mvp` 65건·185점, MVP 스프린트 S02~S13, 네 흐름별 항목, 미루는 것, 종료·데모 체크리스트). §4~§8·§12는 MVP 뒤 계획으로 연기 표시(내용 보존) | #113 | 없음(PRD 수정 없음. MVP 범위는 PRD §12 단계 안의 부분 집합) |
