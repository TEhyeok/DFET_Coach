...
| 관련 에픽·스토리 | EP-00 / DF-901~DF-939(채택 39건), 추가 제안 DF-940·DF-941(채택 대기) |
...
- [3. 카드](#3-카드)
  - P0: [DF-901](#df-901), [DF-902](#df-902), [DF-903](#df-903), [DF-904](#df-904), [DF-905](#df-905), [DF-906](#df-906), [DF-907](#df-907), [DF-908](#df-908), [DF-909](#df-909), [DF-910](#df-910), [DF-911](#df-911), [DF-913](#df-913), [DF-916](#df-916), [DF-926](#df-926), [DF-930](#df-930)
...
| G-09 | 처리방침 게시, 리전·위탁·국외이전 확인(Q-08) | P1a 진입(G-04와 묶음) | DF-909 | — | S08 수령, S09 판정 |
...
| 스프린트 | 항목(점수) | 합계 |
|---|---|---|
| S01 | DF-901(1), DF-902(0), DF-903(1), DF-904(0), DF-908(1), DF-913(0), DF-930(0) | 3 |
| S02 | DF-905(0) | 0 |
| S03 | DF-906(0), DF-909(1) | 1 |
...
**할 일.** 운영 프로젝트에 trainer claim을 가진 테스트 계정 1개와, 그 계정의 `trainers/{uid}.memberIds`에 연결된 가상 회원(실존 인물이 아닌 계정)을 만든다. 에뮬레이터용 시드는 이 항목이 아니다(추가 제안 DF-042 또는 DF-012 범위).
...
**할 일.** storage.rules의 `firestore.get` 교차 조회가 동작하도록 Storage 서비스 에이전트에 Firestore 읽기 권한을 부여한다(콘솔 안내 절차).
...
| Type | owner-action |
| Phase | P0 |
| Sprint | S01 발송(금 10-02), 목표 수령 S08 |
| Points | 1(size/XS) |
| Priority | must |
| Area | privacy |
| Gate | G-04 |
...
**할 일.** 의뢰서를 작성해 발송한다. 질문: Q-04(동의 ④ 범위), Q-05(대기 회원 민감정보), Q-07(동의 증빙 보존), Q-21(프리랜서 트레이너의 처리자 지위), **Q-24(코칭 기록 최대 보유기간 N)**, AS-22(만 14세), AS-31(처리 주체 모델), 대기 회원 최소 정보의 근거, 동의 5종 고지 항목 초안, 국외이전 고지(AS-19). 첨부는 PRD 발췌만 한다. 특허 공개 범위(Q-17) 때문에 PRD 전문과 LiDAR 방법은 보내지 않는다. 회원 실데이터는 넣지 않는다.
...
**늦을 때.** 목표 기준 최신 발송 10-08, 한계 10-30. 12-18까지 개방하지 못하면 P1a 종료 검토를 주 단위로 연기한다(03 §11).
...
**할 일.** S03 첫날 콘솔에서 Firestore·Storage 리전을 확인해 기록한다(Q-08). 처리방침 초안(처리 목적·항목·보유기간, 수탁자 Google LLC(Firebase), 리전 선택이 불가한 서비스의 국외 이전 국가·업체·시기·방법·근거)을 만들어 G-04 법률 검토와 함께 확정·게시한다. 동의 문서가 처리방침 버전을 참조하게 한다(DF-032).

**막는 것 / 막지 않는 것.** DF-925, DF-126의 실제 분석 전송 전환(ADR-015), DF-025의 트리거 리전 확정. **코딩 차단: 아니요**(분석은 DebugSink, 리전은 asia-northeast3 가정).
...
| Depends on | DF-022, DF-035, DF-023, DF-024, DF-025 |
| PRD refs | G-03, ADR-014 |

**할 일.** `rules-YYYYMMDD-N`, `functions-YYYYMMDD-N` 태그를 체크아웃한 상태에서 규칙·인덱스·Storage 규칙과 새 함수(이름 지정)를 배포한다. 명령 형식은 [03 §9.3](../03_RELEASE_AND_SPRINT_PLAN.md#93-규칙인덱스functionsadmin_web)이다. v1 쓰기 차단 스위치는 **아직 끈 상태**로 배포하고 DF-924에서 켠다. 기존 callable 이름·리전은 바꾸지 않는다(NFR-16).
