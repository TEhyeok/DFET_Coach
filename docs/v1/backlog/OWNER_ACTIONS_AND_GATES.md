...
| v1.2 | 2026-09-25 | CJH(AI 에이전트) | DEC-22 MVP 범위(소유자 확인 필요, PR #113): MVP 항목 카드에 `scope/mvp` 라벨과 `### MVP 범위(DEC-22)` 절(지금 만들 것, 미룰 것, 기다리지 않는 의존), MVP 계획에 따라 Sprint 값 변경(원래 계획 병기). DF-925·DF-929 MVP 범위(테스트 회원 전용 플래그 개방)는 소유자 결정이 필요하다. 리뷰 반영: DF-929 MVP 면제 의존에 DF-209, DF-931 MVP 선행 조건 DF-109, 파일 머리 DEC-22 범위 안내. |
