...
final workoutsStreamProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
...
class WorkoutManagerPage extends ConsumerWidget {
              ...
              child: Container(
                ...
                child: workoutsAsync.when(
                  data: (workouts) => workouts.isEmpty
                      ? Center(child: Text('등록된 운동이 없습니다.', style: AdminTheme.bodyMedium))
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              title: Text(workout['name'] ?? '이름 없음', style: AdminTheme.titleMedium),
                              subtitle: Text(
                              ...
                              trailing: Row(
                                ...
                                children: [
                                  IconButton(
                                    ...
                                    onPressed: () => _showEditDialog(context, ref, workout: workout),
                                  ),
                                  IconButton(
  ...
  void _showEditDialog(BuildContext context, WidgetRef ref, {Map<String, dynamic>? workout}) {
    final nameController = TextEditingController(text: workout?['name']);
    final partController = TextEditingController(text: workout?['part']);
    final difficultyController = TextEditingController(text: workout?['difficulty']);

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: AdminColors.surface,
        title: Text(workout == null ? '운동 추가' : '운동 수정', style: AdminTheme.titleLarge),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
              ...
              decoration: const InputDecoration(labelText: '운동 이름', labelStyle: TextStyle(color: Colors.white70)),
            ),
            TextField(
              controller: partController,
              style: const TextStyle(color: Colors.white),
              decoration: const InputDecoration(labelText: '운동 부위', labelStyle: TextStyle(color: Colors.white70)),
            ),
            TextField(
              controller: difficultyController,
              style: const TextStyle(color: Colors.white),
              decoration: const InputDecoration(labelText: '난이도', labelStyle: TextStyle(color: Colors.white70)),
            ),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (workout == null) {
                ...
                await ref.read(contentServiceProvider).updateWorkout(workout['id'], data);
              }
