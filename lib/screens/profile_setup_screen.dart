...
class _ProfileSetupScreenState extends ConsumerState<ProfileSetupScreen> {
  ...
  Future<void> _completeSetup() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        // 프로필이 없으면 새로 생성해야 함 (user_state.dart에서 처리하지만 여기서도 안전장치)
        // userProfileProvider가 null을 반환할 수도 있으므로 체크

        UserProfile? profileToUpdate = userProfile;

        if (profileToUpdate == null) {
          // 프로필이 없는 경우 (매우 드문 케이스, userProfileProvider가 자동 생성함)
          // 여기서는 에러 처리하거나 재시도 로직이 필요할 수 있음
          AppLogger.warning(
              'User profile not found, creating new one locally for update');
        ...
        AppLogger.info('Profile setup completed for ${updatedProfile.email}');
      }

      // main.dart에서 상태 변화 감지하여 자동 전환됨
    } catch (e) {
  ...
  Widget _buildBottomControls() {
    bool isLastPage =
        _currentPage == 6; // Permission page (0-based index: 0,1,2,3,4,5,6)
    // Pages: Goal(0), Gender(1), Age(2), Height(3), Weight(4), Activity(5), Permission(6)
