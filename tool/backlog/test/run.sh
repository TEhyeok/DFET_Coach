...
[ "$c1" -eq 103 ] || fail "first apply created $c1 (expected 103 = 24 epics + 80 items - existing DF-001)"
...
grep -q '^   result: created 0, skipped(existing) 104$' "$work/apply2.txt" || fail "second apply summary"
...
[ "$(wc -l < "$work/created-keys.txt" | tr -d ' ')" -eq 103 ] || fail "issue create calls != 103"
